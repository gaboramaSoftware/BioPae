...
class RegistrosController:
    ...
    def validar_usuario(self, usuario_id: int) -> bool:
        db = SessionLocal()
        try:
    ...
    def validar_usuario_pae(self, usuario_id: int) -> bool:
        db = SessionLocal()
        try:
            usuario = db.query(Usuario).filter(Usuario.id == usuario_id).first()
            return usuario is not None and usuario.es_pae is True
        finally:
            db.close()

    def obtener_datos_alumno(self, usuario_id: int) -> dict | None:
        db = SessionLocal()
        try:
            usuario = db.query(Usuario).filter(Usuario.id == usuario_id).first()
            if not usuario:
            ...
            return {"nombre": usuario.nombre, "rut": usuario.rut, "curso": curso}
        finally:
            db.close()
    ...
    def procesarAsistencia(self, usuario_id: int) -> dict:
        ...
        alumno = self.obtener_datos_alumno(usuario_id)
        return {
            "estado": "Aprobado",
